...
import { AlertTriangle, Building2, Check, Download, FileSpreadsheet, Layers, Search, X } from "lucide-react";
...
}> = ({ titulo, establecimientos, seleccionInicial, soloVencimientosInicial = false, onClose, onExport }) => {
      ...
      <div className="relative w-full max-w-5xl max-h-[92vh] bg-slate-50 rounded-3xl shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col border border-slate-200 overflow-hidden">
          ...
          <div className="p-5 sm:p-6 space-y-6 md:overflow-y-auto">
            ...
            <div>
              ...
              <button
                type="button"
                role="switch"
                aria-checked={soloVencimientos}
                onClick={() => setSoloVencimientos(!soloVencimientos)}
              ...
              >
                <span className="flex items-center gap-2.5 min-w-0">
                  <AlertTriangle className={`h-4 w-4 shrink-0 ${soloVencimientos ? "text-red-500" : "text-amber-500"}`} />
                  <span className="min-w-0">
                    <span className="block text-[12.5px] font-bold text-slate-800">Solo productos vencidos o por vencer</span>
                    <span className="block text-[11px] text-slate-500">Si está apagado, se exporta todo el stock</span>
                  </span>
                </span>
                <span
                  className={`w-9 h-5 rounded-full relative shrink-0 transition-colors ${soloVencimientos ? "bg-teal-600" : "bg-slate-200"}`}
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${
                      soloVencimientos ? "left-[18px]" : "left-0.5"
                    }`}
                  />
                </span>
              </button>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-white flex items-center justify-between gap-4 shrink-0">
          ...
          <button
            ...
            onClick={() => onExport({ ids: elegidosEnLista.map((e) => e.id), modo, soloVencimientos })}
